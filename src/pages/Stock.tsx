...
const ProductModal: React.FC<{ product: any; categories: any[]; onClose: () => void }> = ({ product, categories, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = {
        ...
        variants: formData.variants.map((v: any) => ({ ...v, price: Number(v.price), cost: Number(v.cost) })),
        updatedAt: new Date(),
  ...
  const addVariant = () => {
    setFormData({...formData, variants: [...formData.variants, { name: '', price: 0, cost: 0 }]});
  };
              ...
              <div className="space-y-3">
                {formData.variants.map((v: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-3 bg-slate-50 p-3 rounded-xl border border-slate-100">
                    ...
                    <input required type="number" placeholder="Price" value={v.price} onChange={e => updateVariant(idx, 'price', e.target.value)} className="w-32 px-3 py-2 border border-slate-200 rounded-lg outline-none font-bold text-sm text-emerald-600" />
...
const DealModal: React.FC<{ deal: any; categories: any[]; products: any[]; onClose: () => void }> = ({ deal, categories, products, onClose }) => {
  ...
  const addItem = () => {
    if (products.length === 0) return toast.error('No products available');
          ...
          <form id="deal-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="pt-6 border-t border-slate-100">
              <div className="flex items-center justify-between mb-4">
                ...
                <button type="button" onClick={addItem} className="flex items-center gap-1 text-xs font-bold text-violet-600 bg-violet-50 px-3 py-1.5 rounded-lg hover:bg-violet-100 transition-colors">
              ...
              <div className="space-y-3">
                {formData.items.map((item: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-3 bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <select required value={item.productId} onChange={e => updateItem(idx, 'productId', e.target.value)} className="flex-1 px-3 py-2 border border-slate-200 rounded-lg outline-none font-bold text-sm bg-white">
                    ...
                    <button type="button" onClick={() => removeItem(idx)} className="p-2 text-slate-400 hover:text-rose-500 transition-colors"><Trash2 size={16} /></button>
                  </div>
                ))}
