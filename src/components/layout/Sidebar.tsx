...
const Sidebar: React.FC<SidebarProps> = ({ collapsed: propCollapsed, onToggleCollapsed }) => {
  ...
  const shopName = settings?.shopName || 'Pizza Hut POS';
  ...
  const div1Items = [
    ...
    { name: 'Orders', path: '/sales', icon: History, roles: ['admin', 'cashier'] },
    { name: 'Members', path: '/customers', icon: Users, roles: ['admin', 'cashier'] },
    { name: 'Expenses', path: '/expenses', icon: DollarSign, roles: ['admin', 'cashier'] },
  ];

  const div2Items = [
    /*
    { name: 'Purchase History', path: '/purchases', icon: Truck, roles: ['admin', 'cashier'] },
    { name: 'Inventory Logs', path: '/logs', icon: ClipboardList, roles: ['admin'] },
    { name: 'Warranty', path: '/warranty', icon: Shield, roles: ['admin', 'cashier'] },
    { name: 'RMA / Claims', path: '/rma', icon: RefreshCcw, roles: ['admin', 'cashier'] },
    */
  ];

  const filteredDiv1 = div1Items.filter(item => item.roles.includes(userRole || ''));
  const filteredDiv2 = div2Items.filter(item => item.roles.includes(userRole || ''));
        ...
        <div className="p-6 flex flex-col items-center">
              ...
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 flex items-center justify-center bg-violet-600 rounded-lg shadow-lg shadow-violet-600/20 overflow-hidden">
                  {shopLogo ? (
                    ...
                    <img src="/logo.png" alt="Pizza Hut POS" className="w-full h-full object-contain" />
                  )}
                </div>
          ...
          {!collapsed && (
            <div className="w-full flex flex-col items-center text-center mb-6">
              ...
              <div className={cn(
                "px-3 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5 transition-colors",
                isOnline ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
              )}>
                <div className={cn("w-1.5 h-1.5 rounded-full animate-pulse", isOnline ? "bg-emerald-500" : "bg-amber-500")}></div>
